...
pub struct AssignedRelinearizationKey<
    ...
    const Q: u64,
> {
    value: Vec<(AssignedNTTPoly<F, D, N, Q>, AssignedNTTPoly<F, D, N, Q>)>,
    base: u64,
}
    ...
    AssignedRelinearizationKey<F, D, N, Q>
{
    ...
    pub fn assign(
        ...
        self.value
            .iter()
            .zip_eq(rlk.val.iter())
            .map(|((assigned_rlk_0, assigned_rlk_1), (rlk_0, rlk_1))| {
                assigned_rlk_0.assign(pw, rlk_0.val())?;
                assigned_rlk_1.assign(pw, rlk_1.val())
            })
            .collect::<Result<Vec<()>, Error>>()?;
        Ok(())
    }
