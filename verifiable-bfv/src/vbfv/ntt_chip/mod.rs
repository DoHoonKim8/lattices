...
impl<F: RichField + Extendable<D>, const D: usize> NTTChip<F, D> {
    pub fn ntt_forward(cb: &mut CircuitBuilder<F, D>, input: &Vec<Target>) -> Vec<Target> {
    ...
    pub fn ntt_backward(cb: &mut CircuitBuilder<F, D>, input: &Vec<Target>) -> Vec<Target> {
        let mut current = input.clone();
...
mod tests {
    ...
    fn test_ntt_backward() {
        ...
        let z = NTTChip::ntt_backward(&mut builder, &x);
